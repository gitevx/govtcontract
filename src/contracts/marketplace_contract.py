...
class Product:
    ...
    def application_creation(self):
        return Seq([
            ...
            Assert(Txn.note() == Bytes("govtcontract:uv1")),
            Assert(Btoi(Txn.application_args[3]) > Int(0)),
            App.globalPut(self.Variables.name, Txn.application_args[0]),
            App.globalPut(self.Variables.image, Txn.application_args[1]),
            App.globalPut(self.Variables.document, Txn.application_args[2]),
            App.globalPut(self.Variables.date, Txn.application_args[3]),
            App.globalPut(self.Variables.description, Txn.application_args[4]),
            App.globalPut(self.Variables.price, Btoi(Txn.application_args[5])),
